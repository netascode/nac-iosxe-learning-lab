...
!!! note "Terminology"
    This guide uses **Network-as-Code** (abbreviated as **NAC**) to refer to the automation framework. Technical artifacts like folder names (`nac-iosxe`) and file extensions (`.nac.yaml`) use the lowercase abbreviation.
...
- Writing desired network state configurations in YAML format for IOS XE devices
- Deploying the desired state using Terraform with the NAC modules
