...
```text { .output title="Expected output (clean)" .no-copy }
...
```yaml { title="data/groups/access.nac.yaml" hl_lines="6 7 8" }
access_lists:
  standard:
    - name: AccessLayerACL
      entries:
...
```text { .output title="Expected output (violation)" .no-copy }
cisco@wkst1:~/nac-iosxe$ nac-validate -s .schema.yaml -r rules/ data/
...
```text { .output title="Expected output (clean again)" .no-copy }
cisco@wkst1:~/nac-iosxe$ nac-validate -s .schema.yaml -r rules/ data/
cisco@wkst1:~/nac-iosxe$
```
